...
`cli` for the headless caveat). After editing the manifest, re-sync (see `acme-manifest-and-deploy`).
...
- **Don't invent the wire protocol or a capability the SDK lacks** — the typed SDK is the ceiling on
  ACME. Get facts from the installed package / live doc; `TODO(verify)` gaps. The registry key is
