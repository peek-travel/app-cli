...
| Installations API | **prod** — `https://app-registry.peeklabs.com/installations-api` (`PEEK_API_URL` default) | **sandbox** — `https://app-registry.sandbox.peeklabs.com/installations-api` |
...
| `PEEK_API_URL` | Peek back-office API base | Defaults to the **prod** installations API (`https://app-registry.peeklabs.com/installations-api`). **Sandbox is `https://app-registry.sandbox.peeklabs.com/installations-api`** (what `peek dev` writes to `.env.local`) — it must match the environment the app is registered in |
