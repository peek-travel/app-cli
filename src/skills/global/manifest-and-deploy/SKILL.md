...
what the app plugs into), **registration** in the platform's developer hub, and
...
The manifest declares **what capabilities the app requests** — the **extensions** / extendables
...
- **The app's identity** (its slug) — it's what you push *to*, not part of what you push. One
...
Putting any of them back into the manifest is a rejected push, not a silently ignored key.
...
The CLI also keeps a small **project file** beside the manifest recording which app in the registry
this directory publishes to (and which test app the dev loop uses). It is committed: the slug is
identity, not a secret. Concrete name and fields: `peek-manifest-and-deploy`.
...
- a **dev/test app in sandbox** (created by the CLI's dev command, published at an ephemeral
...
host's env). Don't cross the streams. Note that the two apps share **one** manifest — they differ by
slug and origin, not by file. See `peek-manifest-and-deploy` for the canonical split and dev-CLI
behavior.
