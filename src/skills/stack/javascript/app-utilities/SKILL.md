...
issuer, `Joken` audience — pass the app's `jwtSecret` as `secret`) and **merges both** into one flat
`InstallWebhook`. It throws on any verification failure — one `try/catch` → `401`. The generic model
(and a wire-format example for non-JS apps that must roll their own) is in `webhooks`.

> **Version note (0.7.0).** `parseInstallWebhook` replaced the earlier pair: **`parseInstallEvent` was
> removed** and **`verifyInstallWebhook` is deprecated** (token-only — it can't report
> `accountName`/`platform`/`isTest`). The return is now **flat**: read `event.accountId`, not
> `event.identity.accountId`. Migrate any `parseInstallEvent`/`verifyInstallWebhook` call to
> `parseInstallWebhook(token, body, secret)`.

### The install webhook is where the app learns *who it belongs to*

The install delivery is the **only** source of the account identity — no back-office read and no
peek-auth token returns it. `parseInstallWebhook` returns a flat, JSON-safe `InstallWebhook` meant to
be **persisted as a unit**. Crucially, each field has a **trusted source**: the **verified token** is
authoritative for identity; the **unsigned JSON body** only enriches with name/platform/test, so a
forged body can't override an authenticated id.

| Field | Source | Notes on permanence / use |
| --- | --- | --- |
| `accountId` | **token** | **a.k.a. the partner ID** (two names, one value). **Permanent** — consistent across installs, never changes. **Key any account-permanent data on this**, not on the install id. |
| `installId` | **token** | Identifies a specific install (platform + app + account). Consistent for that install **but may change** — it's the handle you build a server-to-host client from, not an immortal key. |
| `status` / `rawStatus` | **token** | `status` ∈ `installed` / `uninstalled` / `update_installed` (`null` if unrecognized); `rawStatus` is always the wire value. |
| `displayVersion` | **token** | App version at the event. |
| `user` | **token** | Acting user, or `null` for system-initiated events (most uninstalls). |
| `accountName` | body | **Always persist it** — no functional role, but it makes debugging far easier than opaque ids. |
| `platform` | body | `"peek" \| "cng" \| "acme"` — **persist per install.** Decides which access service / APIs / features the install is served by; two installs can differ; this webhook is its only source. `null` if the SDK version doesn't recognize it — **fail loud**, don't default (wrong gateway). |
...
**The identity fields always arrive and are never null — model them as non-nullable DB columns**
(`installId`, `accountId`, `accountName`, `platform`, `isTest`). The `null` on `platform` is *only*
the "SDK version doesn't recognize this value" sentinel you fail loud on and resolve before writing —
never a value you persist. Don't declare these columns nullable.
...
`backoffice-data`. The full persistence model (permanence of each id, `installDataId`) lives in
`webhooks`; the concrete parsers live in your platform's `*-webhooks` skill.
