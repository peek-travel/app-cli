...
export default function SettingsViewPage() {
  ...
  useEffect(() => {
    if (!ready) return;
    let active = true;
    apiFetch<{ name: string }>("/examples/acme/main/api/me")
      .then((data) => {
  ...
  async function pullActivities() {
    ...
    try {
      const data = await apiFetch<{ activities: Activity[] }>(
        "/examples/acme/main/api/activities",
      );
      ...
      <div className="flex w-full max-w-md flex-col items-center gap-6 text-center">
        ...
        <div className="flex flex-col gap-3">
          ...
          <p className="text-sm" style={{ color: "#7c6f9c" }}>
            ...
            >
              app/examples/acme/main/view/page.tsx
            </code>{" "}
            — changes show up here right away, inside ACME.
          </p>
        </div>
